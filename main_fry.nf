...
// run alevin-fry for quantification with splici index
 process alevin_fry_MR3 {
    ...
    then
        alevin-fry generate-permit-list --input ${runId}_ALEVIN_fry_map -d rc  --output-dir ${runId}_ALEVIN_fry_quant --force-cells 50000 --min-reads 10
    else
        alevin-fry generate-permit-list --input ${runId}_ALEVIN_fry_map -d fw --output-dir ${runId}_ALEVIN_fry_quant  --force-cells 50000 --min-reads 10
    fi
...
process velocity {
    ...
    file("/figures/scvelo_proportions_${params.name}_${runId}_veloity.png") into ALEVIN_VELO
    // file("counts_mtx_${protocol}") into PROTOCOL_COUNT_MATRICES


    """
    scVelo_proportions.py ${runId}_ALEVIN_fry_quant ${params.name}_${runId}_veloity.png
    """      
}
